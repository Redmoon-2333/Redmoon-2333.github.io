...
这次的学习材料是一份「双层」代码：`gpt-dev.ipynb` 是我跟着《Let's build GPT》视频一段段敲下来的过程记录（目前停在单头自注意力），`v2.py` 是视频收尾的完成版——多头注意力、前馈层、Transformer Block、6 层堆叠、训练和生成全部组装到位。视频不塞给你一个成品 GPT，而是像搭乐高一样，从最笨的 bigram 出发，每一步只加一个新零件，并且每个新零件都用「已经验证过的笨办法」对照验证一次。这篇笔记按两条线写：先跟着 notebook 走零件，再看 `v2.py` 怎么组装。
...
> **图 8**：完整 GPT 架构教学海报（Faro gpt-image-2.5 生成，数字已逐字核对）。中部为完整数据流：字符编号 → 词义+位置 → Block ×6 → 末层归一化 → lm_head → logits → 采样；左侧小窗是 Block 内部结构速览；右侧两张卡片是超参数与实测参数账（合计 10.79M）；底部一行式总结整个流程。
