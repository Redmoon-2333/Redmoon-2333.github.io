...
缩放初始化调整的是起点，参数更新后，中间量的分布也会变化。我先从 BatchNorm 的公式理解它：用当前 batch 的均值和方差做归一化，再通过可学习的参数缩放、平移。归一化控制均值和方差，并不把任意分布变成正态分布。
...
相同结构、相同初始化、相同 minibatch 序列各训 2000 步，两条 loss 曲线基本交织。我从这次短训练里没有看到明显分离。接下来把 BN 放进更深的网络，再同时看验证损失和饱和率，比较会更具体。
