...
micrograd 是一个 **约 100 行的标量级 autograd 引擎**，用一个紧凑的实现串起计算图、链式法则和参数更新：
...
PyTorch 的 `.backward()` 同样沿计算图反向传播并累加梯度。张量运算和工程实现更复杂，micrograd 则把这一过程展开在标量运算上。
...
**神经元 / 层 / MLP 在这组数据下的具体结构**（权重取一组随机初始化值、保留两位小数，方便手算复核）：
...
loss 从 **6.01 → 0.0051**，预测 `[0.959, -0.992, -0.960, 0.958]` vs 标签 `[1, -1, -1, 1]`。这个 41 参数的 MLP 通过手写 autograd 引擎完成了 4 个样本的分类拟合。
