...
micrograd 是一个 **约 100 行的标量级 autograd 引擎**。对我来说，这个体量刚好能把计算图、链式法则和参数更新连起来看：
...
再回头看 PyTorch 的 `.backward()`，至少我能理解它沿计算图反向传播、累加梯度的基本思路了。张量运算和工程实现要复杂得多，但这次可以先把标量上的过程弄明白。
