...
我先从论文理解词向量为什么要和语言模型一起训练，再把查表、拼接、隐藏层和 softmax 接到字符数据上。
...
**为什么重要**：我在这里重点看 `C` 的共享。同一个词出现在不同位置，都会查到同一行向量；反向传播时，各位置对这行参数的梯度累加起来，方向不必相同。
...
`emb` 是 `(B,3,10)`，要喂给 `W1(30×200)`，需要拼成 `(B,30)`。`torch.cat([emb[:,0,:],emb[:,1,:],emb[:,2,:]],dim=1)` 会创建拼接后的张量；这里的连续存储也允许直接用 `emb.view(B,-1)`。我把 `view` 理解成换一种方式看同一块内存：数据没搬动，变的是形状和步长。这个区别比记住哪一种写法短更有用。
